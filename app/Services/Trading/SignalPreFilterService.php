...
/**
 * SignalPreFilterService — Coin/Timeframe Pre-filter
 *
 ...
 *
 * This service performs deterministic pre-filtering rules (MCP layer):
 ...
 *   8. Return a structured SignalPreFilterResult, or null if the coin should be skipped.
 *
 ...
 */
class SignalPreFilterService
{
    /** @var array<string, SignalPreFilterResult|null> */
    private array $lastMcpResults = [];

    /**
     * Minimum cumulative score required to forward data to the AI service.
     * Set to 1 so pre-filter acts as a soft gate; final signal quality is
     ...
     * and scoring rules, enforces cooldown, and returns a structured SignalPreFilterResult on
     ...
     * @return SignalPreFilterResult|null Structured payload for the AI service, or null to skip
     */
    public function evaluate(string $coin, string $timeframe, string $executionId = ''): ?SignalPreFilterResult
    {
        ...
        $result = new SignalPreFilterResult(
            symbol: $coin,
            timeframe: $timeframe,
    ...
    /**
     * Evaluate pre-filter on configured timeframes and return DTO signals.
     *
    ...
    /**
     * @return array<string, SignalPreFilterResult|null>
     */
    ...
    private function logEvaluation(
        ...
        Log::info('[SignalPreFilterService] Evaluation', [
            'execution_id' => $executionId,
            'symbol' => $coin,
            'timeframe' => $timeframe,
            'rsi' => $rsi,
